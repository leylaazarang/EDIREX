...
First read in information about centers available for this model.
...
lme_fit <- lme(volume ~  center + days  + days:center  + days:Treatment , 
               data = all_data, random = ~ days | GenealogyID)
...
all.equal(names(fitted), all_data$GenealogyID)

fitted <- tibble( MouseID = names(fitted),   center = all_data$center, 
                 Treatment = all_data$Treatment, days = all_data$days, fit_fix = fitted)

```

The following plot depicts the tumor volume increase in different centers for both  treatment groups. 

```{r}
ggplot(fitted, aes(x = days, y = fit_fix, color = center))+
...
```{r}
pvals <- round(sapply(1:n_centers, function(xc)jointFit_centers[[xc]]$Outcome1[["days:TreatmentPhysio", "P"]]), 3)
...
p <- ggplot(data =list_data_JMfit[[xc]], aes(x = day, y = Pred, color = Treatment))+
 ...
 ggtitle(paste(names(list_data_JMfit)[xc]) )+
     annotate("text", x = 5, y = 100, label = paste("p-value:", pvals[[xc]]) )
return(p)
}
)
```
...
By taking  both survival information and tumor volume data into consideration, we observe that in all centers there is a significant effect of treatment on tumor volume growth. Treatment significantly controls tumor growth. 